...
static void parseRequest(char *buf, int fd)
{
	...
	int content_type = 0;
	//GET check part
	char cwd[1024];// PROBLEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEM
	char subbuff[4];
...
static char* returnResponse(int connfd, int error_type, char *filename, int content_type){
	...
	if (error_type == 0) {
	    ...
	    if ((file = fopen(filename, "r")) == NULL) {
		  ...
		  outputBUFF = malloc(sizeof(char) * (strlen(okBUFF) + strlen(source)+ 1));
		  outputBUFF[0] = '\0';
		  strcat(outputBUFF, okBUFF);
		  strcat(outputBUFF, source);
		  fclose(file);
	    }	    
	}
